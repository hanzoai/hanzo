...
export const renderUICurrencyFromJSON = (c, j) => {
  let code = c
  let jsonCurrency = j
  if (code) { code = code.toLowerCase() }
  ...
  const currentCurrencySign = currencySigns[code] != null ? currencySigns[code] : ''
...
export const renderNumericCurrencyFromJSON = (c, j) => {
  let code = c
  let jsonCurrency = j
  if (code) { code = code.toLowerCase() }
...
export const renderJSONCurrencyFromUI = (c, uC) => {
  let code = c
  const uiCurrency = uC ? `${uC}` : '0'
  if (code) { code = code.toLowerCase() }
...
export const isCrypto = (code) => ((code === 'eth') || (code === 'btc') || (code === 'xbt'))

// Convert humanized currency format to data format back to currency format
export const renderUpdatedUICurrency = (c, uiCurrency) => {
  let code = c
  if (code) { code = code.toLowerCase() }
